...
# with open("./附件1：UWB数据集/正常数据/109.正常.txt", "r") as f:  # 打开文件
...
def save_data(file_path: str, Measures):
    with open(file_path, "w+", newline="") as datacsv:
        ...
        csvwriter.writerow(["数据序号", "A0", "A1", "A2", "A3"])
...
def collect_dataset():
    for i in range(1, 325):
        file_path = "./附件1：UWB数据集/正常数据/" + str(i) + ".正常.txt"
        original_data, final_processed_data = data_process(file_path)
        save_data("./附件1：UWB数据集/dataset/normal_data/" + str(i) + ".normal" + ".csv", final_processed_data)

    for i in range(1, 325):
        file_path = "./附件1：UWB数据集/异常数据/" + str(i) + ".异常.txt"
        original_data, final_processed_data = data_process(file_path)
        save_data("./附件1：UWB数据集/dataset/abnormal_data/" + str(i) + ".abnormal" + ".csv", final_processed_data)
...
if __name__ == '__main__':
    # collect_dataset()
    file_path = "./附件1：UWB数据集/异常数据/100.异常.txt"
    original_data, final_processed_data = data_process(file_path)
    save_data("result.csv", final_processed_data)
